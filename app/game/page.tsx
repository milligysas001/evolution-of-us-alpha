...
function initialPeople(leaderName: string, houseName: string, origin: Origin): Person[] {
  ...
  const people: Person[] = [
    { id: "leader", name: leaderName, age: 26 + Math.floor(Math.random() * 12), kin: `ตระกูล ${houseName}`, role: "ผู้นำค่าย", skill: leaderSkill, health: 78 + Math.floor(Math.random() * 12), morale: 65 + Math.floor(Math.random() * 8), fatigue: 0, injured: false, alive: true, traits: ["ผู้ก่อตั้ง", pickFrom(traitPool)] },
  ];
  ...
  for (let i = 0; i < 2; i++) {
    people.push({ id: uid("child"), name: nextName(), age: 4 + Math.floor(Math.random() * 9), kin: i === 0 ? `ตระกูล ${houseName}` : "ชาวบ้าน", role: "เด็ก", skill: "child", health: 58 + Math.floor(Math.random() * 20), morale: 62 + Math.floor(Math.random() * 12), fatigue: 0, injured: false, alive: true, traits: [childTraits[i]] });
  }
...
function addAnnualSettlers(game: GameState, changes: string[]): GameState {
  ...
  if (practicalLimit <= 0 || Math.random() * 100 >= arrivalChance) {
  ...
  const count = 1 + Math.floor(Math.random() * practicalLimit);
...
function addChild(game: GameState): GameState {
  ...
  const child: Person = { id: uid("child"), name, age: 0, kin: `ตระกูล ${game.houseName}`, role: "เด็กแรกเกิด", skill: "child", health: 62 + Math.floor(Math.random() * 15), morale: 70, fatigue: 0, injured: false, alive: true, traits: ["เกิดในค่าย"] };
...
function woundSomeone(game: GameState, cause: string): GameState {
  ...
  const people = game.people.map((p) => p.id === person.id
    ? illness
      ? { ...p, sick: true, health: clamp(p.health - (14 + Math.random() * 16)), fatigue: clamp(p.fatigue + 18), cause }
      : { ...p, injured: true, health: clamp(p.health - (18 + Math.random() * 18)), fatigue: clamp(p.fatigue + 25), cause }
    : p);
...
function applyLeaderFocus(game: GameState, changes: string[]): GameState {
  ...
  if (f === "leadForage") { g = { ...g, resources: changeResources(g.resources, { food: 7, hides: Math.random() < 0.25 ? 1 : 0 }), metrics: changeMetrics(g.metrics, { trust: 2, health: -1 }), threat: clamp(g.threat + 1, 0, 100) }; changes.push("ผู้นำนำคนออกหาอาหารด้วยตนเอง ผลเสริมงานหาอาหารของเดือนนี้และเพิ่มอาหารโดยตรง แต่เพิ่มความเสี่ยงจากป่า"); if (Math.random() < 0.14) { g = woundSomeone(g, "ผู้นำพาคนออกหาอาหารแล้วเกิดอุบัติเหตุในป่า"); changes.push("การออกหาอาหารนำบาดแผลกลับมาด้วย"); } }
  if (f === "boilHerbs") {
    ...
    if (saveText) {
      try {
        ...
        const migrated: GameState = ensureGameState({
          ...
          savedText: "เปิดบันทึกเดิมแล้ว · ปรับข้อมูลเป็นเวอร์ชันใหม่เรียบร้อย",
        } as GameState);
