...
const BUILD_LABEL = "Stabilization · Game Engine · Seeded Random · Save Migration · Schema Validation · Tests";
...
function initialPeople(leaderName: string, houseName: string, origin: Origin): Person[] {
  ...
  let nameIndex = 0;
  const nextName = () => shuffledNames[nameIndex++] ?? `ผู้ตั้งถิ่นฐาน ${nameIndex}`;
  const people: Person[] = [
...
function jobHasAdultWorker(game: GameState, job: LaborKey, assignments: LaborAssignments) {
  ...
  return ids.some((id) => {
    ...
    return !!p && p.alive && p.age >= 15 && p.age < 60 && !personIsInjured(p) && !personIsSick(p) && p.health > 28;
  });
}
...
function hydrateSavedGame(input: unknown): GameState {
  ...
  if (!migrated.checksumValid) throw new Error("Checksum ของไฟล์เซฟไม่ตรงกัน ระบบหยุดโหลดเพื่อป้องกันข้อมูลเสียหาย");
...
function addPerson(game: GameState, role = "ผู้มาใหม่", age?: number): GameState {
  ...
  const picked = names[Math.floor(gameRandom() * names.length)] + ` ${alivePeople(game).length + 1}`;
...
function woundSomeone(game: GameState, cause: string): GameState {
  ...
  const targets = game.people.filter((p) => p.alive && (illness ? !personIsSick(p) : !personIsInjured(p)) && p.age >= 12 && p.health > 25);
...
function advanceMonth(game: GameState): GameState {
  ...
  let nextBase: GameState = { ...g, year: nextYear, month: nextMonth, engineTrace: pipeline.trace as EngineTraceEntry[], recentEventIds: recent, pendingEvents: g.pendingEvents.filter((id) => id !== event.id), currentEventId: "", selectedChoiceId: null, leaderFocus: "workWithPeople", leaderActionSelected: false, savedText: "กำลังจดบันทึก..." };
  ...
  useEffect(() => {
    ...
    for (const candidate of [{ text: saveText, source: "current" }, { text: backupText, source: "backup" }]) {
      ...
      try {
        ...
        const loaded = candidate.source === "backup" ? { ...migrated, savedText: "บันทึกล่าสุดเสียหาย จึงกู้ Autosave ก่อนหน้าให้อัตโนมัติ" } : migrated;
...
function SettingsView({ game, resetGame, showTutorialAgain, theme, setTheme, replaceGame }: { game: GameState; resetGame: () => void; showTutorialAgain: () => void; theme: "light" | "dark"; setTheme: (theme: "light" | "dark") => void; replaceGame: (game: GameState) => void }) {
  ...
  const importSave = () => {
    try {
      ...
      if (!validation.ok) throw new Error(formatValidationIssues(validation.issues));
      window.localStorage.setItem(saveKey, serializeSavedGame({ ...loaded, summaryModal: null, savedText: "นำเข้าเซฟแล้ว" }, { source: "manual-import" }));
      setImportMessage(`นำเข้าเซฟสำเร็จ · Schema ${loaded.schemaVersion ?? CURRENT_SCHEMA_VERSION} · กำลังโหลดใหม่...`);
      window.setTimeout(() => window.location.reload(), 450);
    } catch (error) {
      setImportMessage(`นำเข้าไม่ได้: ${error instanceof Error ? error.message : "รูปแบบ JSON ไม่ถูกต้อง"}`);
    }
  };
  ...
  return (
    <section className="panel pad">
      ...
      <div className="dashboard-grid">
        ...
        <div className="panel kpi"><span className="muted">ระบบบันทึก</span><b>Schema {game.schemaVersion ?? CURRENT_SCHEMA_VERSION}</b><small>Migration + ตรวจโครงสร้าง + Checksum</small></div>
        <div className="panel kpi"><span className="muted">คลังเมือง</span><b>{fmt(game.resources.gold)} 🪙</b><small>ทรัพย์สินเมือง</small></div>
        <div className="panel kpi"><span className="muted">Seed ประจำเกม</span><b title={game.rng?.seed}>{game.rng?.seed ? `${game.rng.seed.slice(0, 28)}${game.rng.seed.length > 28 ? "…" : ""}` : "กำลังสร้าง Seed"}</b><small>สุ่มซ้ำได้ · ใช้ตรวจบั๊กและการแข่งขัน</small></div>
      </div>

      <section className="panel pad save-system-panel" style={{ boxShadow: "none", marginTop: 14 }}>
        <div className="split"><div><h3 className="section-title">ระบบบันทึกสำหรับผู้เล่น</h3><p className="muted small">เกมบันทึกอัตโนมัติพร้อม Checksum และ Schema Version เก็บสำรองก่อนข้ามเดือน รองรับการย้ายเซฟเก่าด้วย Migration ผู้เล่นมีบันทึกด้วยตนเอง 3 ช่อง โหลดกลับ กู้บันทึก หรือดาวน์โหลดไฟล์ได้</p></div><span className="badge green">Autosave เปิดอยู่</span></div>
